<template>
  <div class="w-full h-full bg-white text-black">
    <div class="p-8">
      <h1 class="text-2xl rounded-t-lg font-bold text-gray-500 mb-6">Patient List</h1>
      <table class="min-w-full bg-white border rounded-xl border-gray-300">
        ...
        <tbody>
          <template v-for="patient in patients" :key="patient.name">
            <tr class="hover:bg-gray-100x cursor-pointer" @click="togglePatient(patient)">
            ...
            <tr v-if="selectedPatient && selectedPatient.name.trim().toLowerCase() === patient.name.trim().toLowerCase()"
